...
namespace sai_OSCController
{
    ...
    public class FacialManager
    {
        ...
        public float eyeBallSens = 1f;
        public float browSens = 1.1f;
        public float browSmooth = 0.1f;
        public float mouthRightLeftSens = 1.1f;
        public float mouthOpenCloseSens = 1.1f;
        ...
        Vector2 leftEyeRot = new();
        Vector2 rightEyeRot = new();
        ...
        Clips tongueOut = new("tongueOut", 0f);
        #endregion

        public FacialManager()
        {
            ...
            Console.WriteLine("FacialManager Start");
        }

        public void Update(Dictionary<string, float> ReceivedValue)
        {
            ...
            var rotationX = ReceivedValue["Rotation_X"];
            var rotationY = ReceivedValue["Rotation_Y"];
            var rotationZ = ReceivedValue["Rotation_Z"];

            //目
            {
                var valueBrow = LastEyeBrow.LerpUnclamped(browInnerUp.Value * browSens, browSmooth);
                var valueBrowR = LastEyeBrowR.LerpUnclamped(browDownLeft.Value * browSens, browSmooth);
                var valueBrowL = LastEyeBrowL.LerpUnclamped(browDownRight.Value * browSens, browSmooth);
            ...
            //眼球の回転
            {
                ...
                // 目の回転角度を最大値に制限する
                leftEyeX = Math.Clamp(leftEyeX, -eyeBallMaxAngle, eyeBallMaxAngle);
                leftEyeY = Math.Clamp(leftEyeY, -eyeBallMaxAngle, eyeBallMaxAngle);
                leftEyeZ = Math.Clamp(leftEyeZ, -eyeBallMaxAngle, eyeBallMaxAngle);
                rightEyeX = Math.Clamp(rightEyeX, -eyeBallMaxAngle, eyeBallMaxAngle);
                rightEyeY = Math.Clamp(rightEyeY, -eyeBallMaxAngle, eyeBallMaxAngle);
                rightEyeZ = Math.Clamp(rightEyeZ, -eyeBallMaxAngle, eyeBallMaxAngle);

                // 目の回転角度を保存する
                leftEyeRot = new Vector2(leftEyeX * eyeBallSens, leftEyeY * eyeBallSens);
                rightEyeRot = new Vector2(rightEyeX * eyeBallSens, rightEyeY * eyeBallSens);
            }

            // 送信
            List<OscMessage> messages = new List<OscMessage>();
            messages.Add(new("/avatar/parameters/FTLeftEye", eyeBlinkLeft.Value));
            messages.Add(new("/avatar/parameters/FTRightEye", eyeBlinkRight.Value));
            messages.Add(new("/avatar/parameters/FTMouth_A", jawOpen.Value));
            messages.Add(new("/avatar/parameters/FTMouth_I", mouthLowerDownRight.Value));
            messages.Add(new("/avatar/parameters/FTMouth_U", mouthFunnel.Value));
            messages.Add(new("/avatar/parameters/FTMouth_E", mouthLowerDownRight.Value));
            messages.Add(new("/avatar/parameters/FTMouth_O", mouthPucker.Value));
            messages.Add(new("/avatar/parameters/FTLeftEyeHorizontal", (leftEyeRot.Y * -1) + 0.3f));
            messages.Add(new("/avatar/parameters/FTLeftEyeVertical",eyeLookUpLeft.Value));
            messages.Add(new("/avatar/parameters/FTLeftBrowUp", browInnerUp.Value));
            messages.Add(new("/avatar/parameters/FTLeftBrowDown", browDownLeft.Value));
            messages.Add(new("/avatar/parameters/FTRightBrowUp", browInnerUp.Value));
            messages.Add(new("/avatar/parameters/FTRightBrowDown", browDownRight.Value));
            messages.Add(new("/avatar/parameters/FTSmile", mouthSmileLeft.Value));
